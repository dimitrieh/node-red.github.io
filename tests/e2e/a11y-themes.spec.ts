...
test('no horizontal scroll at 320px viewport on landing pages', async ({ page }) => {
  ...
  for (const path of ['/', '/docs/', '/blog/', '/about/']) {
    ...
    expect(
      dims.sw,
      `${path} horizontal scroll (sw=${dims.sw} > cw=${dims.cw})`,
    ).toBeLessThanOrEqual(dims.cw + 1);
  }
});
  ...
  for (const theme of ['light', 'dark'] as const) {
    await page.evaluate((t) => {
      try {
        ...
        // The data-theme attribute set on the next line is what this test
        // actually asserts against; persisting to storage is only there so a
        // navigation inside the test keeps the theme.
      }
      document.documentElement.setAttribute('data-theme', t);
    }, theme);
