...
test.describe('Homepage round-2 content restoration', () => {
  ...
  test('Trusted-by section carries the permission disclaimer + invite link', async ({ page }) => {
    ...
    await expect(page.locator('.trusted-disclaimer')).toContainText(/All logos used with permission/);
  });
  ...
  test('Get Started section links to the full platforms index', async ({ page }) => {
    ...
    await expect(more.locator('a[href="/docs/getting-started/"]')).toContainText(/see all platforms/i);
  });
...
test.describe('Homepage social-share metadata', () => {
  test('Per-page og:image and twitter:image fall back to node-red-icon.png on /', async ({ page }) => {
    await page.goto('/');
