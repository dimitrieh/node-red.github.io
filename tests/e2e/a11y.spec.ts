 ...
 *
 * - Writes a JSON report to reports/audit-qa-a11y.json with the per-page
 ...
 *   of in-flight items, each entry documented with its root cause below. New
...
/**
 * Known-open serious violations, kept here rather than in a separate tracker so
 * the allowlist and its justification stay together. They collapse to two root
 * causes:
 *
...
test('write a11y report', async () => {
  const out = resolve(process.cwd(), 'reports/audit-qa-a11y.json');
  ...
  expect(criticalCount, `Expected 0 critical-impact a11y violations; got ${criticalCount}.`).toBe(
    0,
  );
