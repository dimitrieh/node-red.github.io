...
<!-- Every property below was a kramdown definition list: a bare term line followed by
     `: definition`. Kramdown turned that into <dl>/<dt>/<dd>; remark, Astro's markdown parser,
     has no definition-list support, so it glued term and definition into one paragraph and
     left a literal ": " in front of every description. Written out as explicit
     <dl>/<dt>/<dd>, which Astro passes through untouched.
     Two details are load-bearing. The blank line after each <dd>, and before each </dd>,
     closes the raw HTML block so the definition body is still parsed as markdown. And the
     bodies are dedented by two spaces relative to the kramdown source, which puts the
     example blocks at exactly four spaces: still indented code blocks, but without the two
     stray leading spaces that showed up inside every <pre> while the definitions were being
     parsed as top-level paragraphs.
     Where the <dl> closes and reopens partway through a section, that is deliberate and
     matches the pre-migration output: content indented less than the definition body ended
     the kramdown list too. -->
<dl>
<dt>flowFile</dt>
<dd>
...
<dl>
<dt>functionExternalModules</dt>
<dd>

if set to `true`, the Function node's Setup tab will allow adding additional modules that will become available to the function. See [Writing Functions](/docs/user-guide/writing-functions#using-the-functionexternalmodules-option) for more information. Default: `false`.

</dd>
<dt>functionTimeout</dt>
<dd>
