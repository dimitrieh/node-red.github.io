...
information about writing Functions is available [here](/docs/user-guide/writing-functions).
...
<!-- inline-block so the block shrink-wraps and sits beside the floated image above -->
...
<em>Note:</em> the parts array may contain additional meta-data about the sequence. For example,
the <code>split</code> node also attaches information that can be used by the <code>join</code> node to
reassemble the sequence. See the <code>split</code> node's documentation.
</div>
